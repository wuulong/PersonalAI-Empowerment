...
當小明在第 14.3 節成功打通了 CLI 直連腳本與 SQLite 數據持久化後，他意識到：「這套『文獻自動化探勘 -> 物理參數動態 JSON 提取 -> 紅軍自審脆弱點定錨 -> SQLite 結構化落庫』的核心邏輯，應該被封裝成一個通用的全域技能，供我未來面對任何新課題時一鍵調用。」
...
*   **拒絕黑箱代碼**：AI 被禁止直接提供完整、不可讀的代碼段。
*   **強迫蘇格拉底式引導**：AI 在給出代碼或物理推導修正前，**必須先向學生提問**：「你認為當壓電常數 $d_{33}$ 發生退化時，你的電阻匹配迴路需要做什麼相應調整？」
